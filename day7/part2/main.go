...
func isFiveOfAKind(cardCounts map[rune]int) bool {
	for r, count := range cardCounts {
		if count == 5 || (r != 'J' && count+cardCounts['J'] >= 5) {
			return true
		}
	}
	return false
...
func isFourOfAKind(cardCounts map[rune]int) bool {
	for r, count := range cardCounts {
		if count == 4 || (r != 'J' && count+cardCounts['J'] >= 4) {
			return true
		}
	}
	return false
...
func isThreeOfAKind(cardCounts map[rune]int) bool {
	for r, count := range cardCounts {
		if count == 3 || (r != 'J' && count+cardCounts['J'] >= 3) {
			return true
		}
	}
	return false
...
func isTwoPairs(cardCounts map[rune]int) bool {
	...
	return pairCount == 2 || pairCount == 1 && cardCounts['J'] == 1 || cardCounts['J'] == 2
}

func isOnePair(cardCounts map[rune]int) bool {
	for _, count := range cardCounts {
		if count == 2 || cardCounts['J'] == 1 {
			return true
		}
	}
	return false
